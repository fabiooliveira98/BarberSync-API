const BarbeiroModel = require('../models/barbeiro.model');
const AgendamentoModel = require('../models/agendamento.model');

class BarbeiroController {
  listar(req, res) {
    BarbeiroModel.findAll((err, linhas) => {
  ...
  criar(req, res) {
    ...
    BarbeiroModel.create({ nome, especialidade }, function(err) {
      if (err) return res.status(500).json({ erro: err.message });
  ...
  disponibilidade(req, res) {
    ...
    AgendamentoModel.findOccupiedHours(id, data, (err, agendamentos) => {
      if (err) return res.status(500).json({ erro: err.message });
